...
Namespace CustomDraw
    Partial Public Class Form1
        ...
        Private startPosition_Renamed As PdfDocumentPosition
        ...
        Private Sub pdfViewer1_MouseDown(ByVal sender As Object, ByVal e As MouseEventArgs)
            If e.Button = MouseButtons.Left Then
                startPosition_Renamed = pdfViewer1.GetDocumentPosition(e.Location)
                endPosition = Nothing
        ...
        Private Sub pdfViewer1_Paint(ByVal sender As Object, ByVal e As PaintEventArgs)
            ...
            If startPosition_Renamed IsNot Nothing AndAlso endPosition IsNot Nothing Then
                Dim startPoint As PointF = pdfViewer1.GetClientPoint(startPosition_Renamed)
                Dim endPoint As PointF = pdfViewer1.GetClientPoint(endPosition)

                g.FillRectangle(New SolidBrush(Color.FromArgb(128, Color.Aqua)), RectangleF.FromLTRB(Math.Min(startPoint.X, endPoint.X), Math.Min(startPoint.Y, endPoint.Y), Math.Max(startPoint.X, endPoint.X), Math.Max(startPoint.Y, endPoint.Y)))
            End If
        End Sub
    End Class
End Namespace
